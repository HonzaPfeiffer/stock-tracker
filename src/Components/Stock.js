...
class Stock extends Component {
  ...
  async componentDidMount () {
    ...
    if (stockSymbol.indexOf('.') !== -1) {
      ...
      for (let index = 0; index < querys.length; index++) {
        try {
          const response = await fetch('https://alpha-vantage.p.rapidapi.com/query?' + querys[index] + '&symbol=' + this.state.stock[0] + '&datatype=json&output_size=compact', {
            method: 'GET',
            headers: {
              'x-rapidapi-key': tokens.tokens[index],
  ...
  transformData (objects) {
    ...
    for (let index = Object.values(objects).length - 1; index >= 0; index--) {
      ...
      const priceObject = Object.values(objects)[index]
